...
// Update to your actual route file path
...
describe('GET /api/drive/download', () => {
  ...
  it('exports Google Doc to DOCX and returns buffer with headers', async () => {
    ...
    const res = await GET(
      createRequest('http://localhost/api/drive/download?id=123&name=custom_name'),
    );
    ...
    expect(res.headers.get('Content-Disposition')).toContain('custom_name.docx');
  ...
  it('streams regular files directly', async () => {
    ...
    const res = await GET(createRequest('http://localhost/api/drive/download?id=456'));
  ...
  it('returns 401 if not authenticated', async () => {
    ...
    const res = await GET(createRequest('http://localhost/api/drive/download?id=789'));
  ...
  it('returns 500 on unexpected error', async () => {
    ...
    const res = await GET(createRequest('http://localhost/api/drive/download?id=999'));
