...
describe('UploadModal', () => {
  ...
  it('opens and closes modal', async () => {
    ...
    expect((await screen.findAllByText(/upload files/i))[0]).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: '' })); // close icon
    await waitFor(() => expect(screen.queryByText('Trigger Success')).not.toBeInTheDocument());
  });
  ...
  it('closes modal on all success', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('Trigger All Success')).not.toBeInTheDocument());
  });
});
